...
const PYTHON_BACKEND_URL =
  process.env.PYTHON_BACKEND_URL || "https://argus.railway.app";
  ...
  try {
    ...
    if (!geminiApiKey) {
      ...
      return NextResponse.json({
        ...
        description:
          "Basic video analysis mode - add GEMINI_API_KEY for enhanced features",
        duration: 600,
        sections: getDefaultSections(600),
        transcript:
          "Limited analysis mode - Gemini API key required for advanced video understanding",
        processed: true,
      });
    }
    ...
    try {
      ...
      if (youtubeApiKey) {
        try {
          ...
          console.warn(
            "YouTube API error, continuing with basic metadata:",
            apiError
          );
        }
      }
      ...
      if (extractFrames) {
        try {
          ...
          if (framesResponse.ok) {
            ...
            console.log(
              `Extracted ${keyFrames.length} key frames for analysis`
            );
          }
        } catch (frameError) {
  ...
  "technicalConcepts": ["concept 1", "concept 2", ...]
      ...
      try {
        ...
        geminiAnalysis = {
          ...
          sections: generateFallbackSections(
            videoMetadata.title,
            videoMetadata.description,
            videoMetadata.duration
          ),
          keyTopics: extractTopicsFromText(
            videoMetadata.title + " " + videoMetadata.description
          ),
          summary: "AI-generated analysis of video content",
          visualElements:
            keyFrames.length > 0 ? ["Visual content analyzed"] : [],
          technicalConcepts: [],
        };
      }

      // Process sections
      const sections = geminiAnalysis.sections.map(
        (section: any, index: number) => ({
          id: `section-${index + 1}`,
          title: section.title || `Section ${index + 1}`,
          startTime: Math.max(
            0,
            Math.min(section.startTime || 0, videoMetadata.duration)
          ),
          endTime: Math.max(
            0,
            Math.min(
              section.endTime || videoMetadata.duration,
              videoMetadata.duration
            )
          ),
          description:
            section.description || "AI-generated section description",
        })
      );
      ...
      const processedVideoData = {
        ...
        sections: sections,
        transcript:
          transcript || `AI-enhanced analysis for "${videoMetadata.title}"`,
      ...
      return NextResponse.json({
        ...
        description:
          "Unable to process with Gemini AI - using fallback analysis",
        duration: 600,
        sections: getDefaultSections(600),
...
function extractTopicsFromText(text: string): string[] {
  ...
  return topics.length > 0
    ? topics
    : ["General Content", "Information", "Discussion"];
}
